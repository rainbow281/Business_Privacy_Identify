...
class CCF2020DataProcess(object):
    def __init__(self, config):
        ...
        # pku_seg：中文分词工具包（器）
        self.pku_seg = None
        # 将文本拆分为单词（标记）的过程称为标记化，工具为标记器tokenizer
        self.tokenizer = None
    ...
    def format_data(self, data_type='train', num_works=1):
        if self.thu_lac is None or self.pku_seg is None or self.tokenizer is None or self.word_w2v is None:
            # 实例化一个pkuseg分词器（加载模型及用户词典）
            self.pku_seg = pkuseg.pkuseg()
            # 实例化一个标记器：用BertTokenizer对输入文本进行处理，从预训练模型中加载tokenizer
        ...
        clue_data = json_load_by_line(ROOT_DATA + 'clue\\train_dev_test_data.json')
        ...
        def convert_to_token(sample):
            # my_encode中调用encode方法，encode中调用了tokenize方法，
            # 所以在使用的过程中，我们可以通过设置encode方法中的参数，达到转化数据到可训练格式一步到位的目的
        ...
        # 主要处理：使用以上三个内置函数（拆分文本、获取词、转换为token标记）
...
# 将text_dir（+label_dir）中数据文本以{'id': None, 'text': None, 'entities': []}形式以json类型写入output_file
def convert2json(text_dir, output_file, label_dir=None):
    # os.listdir() 方法用于返回指定的文件夹包含的文件或文件夹的名字的列表。打印输出为文件名
    text_file_list = os.listdir(text_dir)
    data = []

    # 将所有txt文件以{'id': None, 'text': None, 'entities': []}形式添加到data
    for text_file in text_file_list:
        d = {'id': None, 'text': None, 'entities': []}
        # with…as语句是简化版的try except finally语句，可以“实现稳定性和灵活性更好的设计”
        with open(text_dir + text_file, 'r', encoding='utf-8') as fp:
            d['text'] = fp.read()
            # 从右往左从-1开始，从左往右从0开始，左闭右开
            d['id'] = int(text_file[:-4])
            # d['id'] = int(text_file.split(',')[:-4])
        data.append(d)

    # 只有train_data中有label_dir
    # 将data中的entities用label中的隐私信息、种类、开始位置、结束位置进行填充
    if label_dir is not None:
        for d in data:
            # "{}.csv".format(d['id'])：format中只有一个参数，{}处放第一个参数，举例：1.csv
            ...
            for item in reader:
                ...
                if reader.line_num == 1:
                    continue
                # 因为起始和结束位置为全闭区间，所以做切片需要加一
                # 如果label中和文本结果的实体不一致，则输出：标签实体错误
                ...
                entity = {'privacy': item[4],
                          ...
                          'pos_b': int(item[2]),
                          'pos_e': int(item[3])}
                # 如果文本ID和label中的ID不一致，则输出：两个id
                if not d['id'] == int(item[0]):
                    print(d['id'], int(item[0]))
                # 将data中的每个字典中entity填充label的隐私信息、种类、开始位置、结束位置
                d['entities'].append(entity)

    # key=lambda 元素: 元素[字段索引]，默认reverse为false升序，sort为排序函数
...
def conv2json():
    convert2json(ROOT_LOCAL_DATA + 'train_data\\data\\', ROOT_LOCAL_DATA + 'train_data.json',
                 label_dir=ROOT_LOCAL_DATA + 'train_data\\label\\')
    convert2json(ROOT_LOCAL_DATA + 'test_data\\', ROOT_LOCAL_DATA + 'test_data.json')
    # convert2json(ROOT_LOCAL_DATA+'', ROOT_LOCAL_DATA + 'train_data.json',
    #              label_dir=ROOT_LOCAL_DATA + 'train_label\\')
    # convert2json(ROOT_LOCAL_DATA+'', ROOT_LOCAL_DATA + 'test_data.json')
