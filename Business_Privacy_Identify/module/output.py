...
# 便签编码有四种：'Nobie', 'Tobie', 'Cobie', 'point'
...
class OutputConfigObie(OutputConfig):
    ...
    def __str__(self):
        ...
        # 2.使用“super().方法名”调用并不是用于直接调用当前类的父类的方法，
        # 而是Python根据MRO算法去所有父类中查找找到的第一个与“方法名”同名的某个父类方法，
        # 因此如果某子类继承多个超类、或某子类有多个层级的超类时，这些超类中有多个同名的方法时，只会执行一次；
